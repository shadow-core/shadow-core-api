...
import { ExpressCoreUsers } from 'express-core-users';
...
export default class UsersController extends BasicController {
    ...
    async signupUserAction(req, res) {
        ...
        let newUser = await this.core.ProcessSignUpUser(_action_params.email, _action_params.password);
        //sendVerificationEmail(newUser);
        return this.returnSuccess(this.core.getJsonAnswer('signup_user', 'success'), res);
    }
    ...
    async verifyEmailResendAction(req, res) {
        ...
        let user = await this.core.getUserByEmail(_action_params.email);

        //return error if too much requests
        if (this.core.checkTooMuchRequests(user)) {
            return this.returnError(this.core.getJsonAnswer('verify_email_resend', 'too_much_requests'), res, 429);
        }

        //send email, add counter and return success
        await this.core.updateResendVerification(user);
        //sendVerificationEmail(user);
        return this.returnSuccess(this.core.getJsonAnswer('verify_email_resend', 'success'), res);
    }
    ...
    async verifyEmailAction(req, res) {
        ...
        //find user by token
        let user = await this.core.getUserByVerificationToken(_action_params.verification_token);
        if (!user) {
            return this.returnNotFoundError(res, 'verification_token', 'Email verification token is incorrect or outdated');
        } else {
            user.isEmailVerified = true;
            await user.save();
            return this.returnSuccess(this.core.getJsonAnswer('verify_email', 'success'), res);
        }
    }
    ...
    async resetPasswordRequestAction(req, res) {
        ...
        if (this.core.checkTooMuchResetPasswordRequests(user)) {
            return this.returnError(this.core.getJsonAnswer('reset_password_request', 'error_reset_password_too_much_requests'), res, 429);
        }

        //seems like we can send reset password
        await this.core.prepareResetPassword(user);
        //sendResetPasswordRequestEmail(user);
        return this.returnSuccess(this.core.getJsonAnswer('reset_password_request', 'success'), res);
    }
    ...
    async resetPasswordAction(req, res) {
        ...
        let user = await this.core.getUserByPasswordResetToken(_action_params.token);
        if (!user) {
        ...
        return this.returnSuccess(this.core.getJsonAnswer('reset_password', 'success'), res);
    }
    ...
    async resetPasswordCheckAction(req, res) {
        ...
        return this.returnSuccess(this.core.getJsonAnswer('reset_password_check', 'success'), res);
    }
}